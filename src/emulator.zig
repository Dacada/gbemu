...
pub fn Emulator(Cpu: type, Apu: type, Ppu: type, Timer: type, Scheduler: type, Debugger: type) type {
    return struct {
        ...
        pub fn tick(self: *This) !bool {
            // Step the PPU first thing on the T-cycle. It needs to know which T-cycle it's on so it can accurately trigger the effects of the OAM bug at the start of the next M-cycle
